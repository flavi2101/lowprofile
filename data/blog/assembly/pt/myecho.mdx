...
Imprimi na tela o que voce digitou no terminal ( que explicação incrivel e detalhada).
...
Algums pontos relevantes antes de iniciar o codigo do "echo":
 ...
 - Uma instrução em assembly é definida por um conjunto de três informações: um mnemônico que representa a ação que o processador irá executar, e dois operandos, um de destino e um de origem.
...
> <p style={{color:'greenyellow'}}>  Caso prefira um jeito mais direto, é mais simples pesquise na internet: 'System call x86 table linux'. <span hidden='true'>[@chromiumSyscalls]</span> </p> 
...
Vamos acha essa header[^6] e ver seu código.
[^6]: São arquivos que na fase de pre processamento do compilador seu contúdo (declarações de funçoes, macros...) que precisam ser compartilhados por multiplos arquivos e são literalmente compiados para o arquivo onde a chamda **#incude** foi inserida.
